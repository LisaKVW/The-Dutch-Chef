...
const CreateRecipeWithImg = async (req, res) => {
  try {
    ...
    const recipeImgs = await Promise.all(createdRecipes.map((recipe, i) => { // to get the images, we need to map through all our recipes, this as the images are in the 2nd object of each recipe array
